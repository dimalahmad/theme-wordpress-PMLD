...
if ($is_dummy && $dummy_data) {
    ...
    $stock = isset($dummy_data['stock']) ? $dummy_data['stock'] : 0;
    ...
    $gallery_ids = get_post_meta(get_the_ID(), '_sparepart_gallery', true);
    $gallery_ids = $gallery_ids ? explode(',', $gallery_ids) : array();
    $specifications = get_post_meta(get_the_ID(), '_sparepart_specifications', true);
...
if ($is_dummy || (isset($title) && $title)) :
    ...
    $review_sparepart_id = $is_dummy ? 'dummy_' . $dummy_id : get_the_ID();
