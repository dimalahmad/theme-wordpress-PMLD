...
function inviro_product_price_callback($post) {
    ...
    $price_raw = get_post_meta($post->ID, '_product_price', true);
    $original_price_raw = get_post_meta($post->ID, '_product_original_price', true);
...
function inviro_save_product_meta($post_id) {
    ...
    if (isset($_POST['product_original_price']) && !empty($_POST['product_original_price'])) {
    ...
    if (isset($_POST['product_price']) && !empty($_POST['product_price']) && $_POST['product_price'] > 0) {
        ...
        update_post_meta($post_id, '_product_price', 0);
    }
