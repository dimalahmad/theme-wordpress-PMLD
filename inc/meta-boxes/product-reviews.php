 ...
 */
function inviro_add_product_review_meta_boxes() {
    add_meta_box(
        'product_review_info',
        'Informasi Ulasan Produk',
        'inviro_product_review_info_callback',
        'product_review',
        'side',
        'default'
    );
    ...
    add_meta_box(
        ...
        'Status Ulasan',
        'inviro_product_review_status_callback',
        'product_review',
        'side',
        'high'
    );
}
add_action('add_meta_boxes', 'inviro_add_product_review_meta_boxes');

function inviro_product_review_info_callback($post) {
    wp_nonce_field('inviro_product_review_meta', 'inviro_product_review_meta_nonce');
    $product_id     = get_post_meta($post->ID, '_review_sparepart_id', true); // reuse meta key for consistency
    ...
    <div style="padding: 10px 0;">
        ...
        <p style="margin-bottom: 8px;"><strong>Produk (ID Produk):</strong></p>
        <input
            type="number"
        ...
        <p class="description" style="margin-top: 8px; font-size: 13px;">
            ...
            if ($product_id) {
                ...
                if ($product) {
                    echo 'Produk: <a href="' . get_edit_post_link($product_id) . '" target="_blank">' . esc_html($product->post_title) . '</a>';
                } else {
                    echo 'ID tidak ditemukan';
                }
            } else {
                echo 'Masukkan ID Produk yang akan diulas';
            }
            ?>
        </p>
    </div>
...
function inviro_product_review_status_callback($post) {
...
function inviro_save_product_review_meta($post_id) {
    if (get_post_type($post_id) !== 'product_review') {
        return;
    }
    
    if (
        !isset($_POST['inviro_product_review_meta_nonce']) ||
        !wp_verify_nonce($_POST['inviro_product_review_meta_nonce'], 'inviro_product_review_meta')
    ) {
        return;
    }
    ...
    if (isset($_POST['review_product_id'])) {
        update_post_meta($post_id, '_review_sparepart_id', absint($_POST['review_product_id']));
        update_post_meta($post_id, '_review_is_dummy', '0');
        update_post_meta($post_id, '_review_product_type', 'produk');
    }
...
add_action('save_post_product_review', 'inviro_save_product_review_meta');
